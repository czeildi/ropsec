...
describe("sign_commits_with_key", {
  ...
  it("sign_commits_with_key returns key if existing key", {
  ...
  it("global arg in sign_commits_with_key passed to set_key_to_sign_commits", {
  ...
  it("error in sign_commits_with_key if multiple keys for given name, email pair", {
    mockery::stub(sign_commits_with_key, "get_key_candidates", data.frame(id = 1:3))
    expect_error(sign_commits_with_key())
  })

  it("commits are signed with found existing key", {
    sign_mock <- mockery::mock()
    mockery::stub(sign_commits_with_key, "set_key_to_sign_commits", sign_mock)
    mockery::stub(
  ...
  it("if neither key nor name provided, name is extracted from git config", {
  ...
  it("if neither key nor email provided, email is extracted from git config", {
    git_mock <- mockery::mock()
    mockery::stub(sign_commits_with_key, "find_git_option", git_mock)
    mockery::stub(sign_commits_with_key, "get_key_candidates", data.frame())
    mockery::stub(sign_commits_with_key, "generate_key_with_name_and_email", "id1")
  ...
  it("if neither key nor email provided, email is extracted from git config and used to generate new key", {
  ...
  it("if one existing key found, it is communicated in message", {
  ...
  it("error is thrown if email is neither provided nor available in git config", {
...
describe("set_key_to_sign_commits", {

  it("if everything already set, it is communicated and no further confirmation needed", {
  ...
  it("if user did not confirm, git2r config not called and NULL returned", {
  ...
  it("if user did confirm, git2r config is called with appropriate params and key returned", {
    mockery::stub(set_key_to_sign_commits, "extract_git_option", "gpg")
  ...
  it("gpg program is set at first time", {
...
describe("assemble_confirmation_message", {
  it("asks for confirmation with meaningful message", {
...
describe("filter_keys_on_name_and_email_if_provided", {
  it("no matching keys returned if initial df is empty", {
  ...
  it("no matching keys returned if both ids are NULL", {
  ...
  it("matching keys returned if one id is NULL", {
  ...
  it("matching key returned if both ids provided", {
