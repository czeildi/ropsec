...
- globally: `git config commit.gpgsign false`

## Attempt to fix

- check that `git config gpg.program` matches your installed version
- based on [this](https://unix.stackexchange.com/questions/382279/gpg-hangs-when-private-keys-are-accessed) post:
  - `pkill -9 gpg-agent`
  - set env var: `GPG_TTY=$(tty)` (with `export GPG_TTY=$(tty)` in `~/.bashrc` or similar)
  - commit from command line first and provide passphrase of GPG key
