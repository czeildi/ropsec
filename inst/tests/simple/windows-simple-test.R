...
test_that("ssh directory exists", {
  expect_true(file.exists(fs::path_expand("~/.ssh")))
})

context("SSH Configuration - keys")
test_that("ssh has public/private keys", {
  expect_true(file.exists(fs::path_expand("~/.ssh/id_rsa")))
  expect_true(file.exists(fs::path_expand("~/.ssh/id_rsa.pub")))
})
...
test_that("~/.Rprofile permissions are sane", {
  if (file.exists(fs::path_expand("~/.Rprofile"))) {
    expect_true(file.info(fs::path_expand("~/.Rprofile"))$mode == structure(416L, class = "octmode"))
  }
})

context("~/.Renviron permissions are sane (if ~/.Renviron exists)")
test_that("~/.Renviron permissions are sane", {
  if (file.exists(fs::path_expand("~/.Renviron"))) {
    expect_true(file.info(fs::path_expand("~/.Renviron"))$mode == structure(416L, class = "octmode"))
  }
})
